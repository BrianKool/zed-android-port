...
impl AgentApiKeyModal {
    fn new(
        ...
        let api_key_input = cx.new(|cx| {
            InputField::new(window, cx, "Paste API key")
                .label("API key")
                .masked(true)
                .tab_stop(true)
        });
        window.focus(&api_key_input.focus_handle(cx), cx);
        Self {
        ...
        v_flex()
            ...
            .child(
                h_flex().w_full().justify_end().child(
                    Button::new("save-agent-api-key", "Save")
                        .style(ButtonStyle::Tinted(TintColor::Accent))
                        .on_click(cx.listener(|this, _, _, cx| {
                            let api_key = this.api_key_input.read(cx).text(cx);
                            if api_key.trim().is_empty() {
                                return;
                            }
                            this.provider
                                .set_api_key(Some(api_key), cx)
...
enum AuthState {
    ...
    Unauthenticated {
        ...
        configuration_view: Option<Arc<dyn language_model::LanguageModelProvider>>,
        pending_auth_method: Option<acp::AuthMethodId>,
        _provider_subscription: Option<Subscription>,
    },
}
...
impl ConversationView {
    ...
    fn handle_auth_required(
        ...
        let provider_id = if cfg!(target_os = "android") {
            match agent_id.as_ref() {
        ...
        let (configuration_view, subscription) = if let Some(provider_id) = &provider_id {
            ...
            let view = registry.read(cx).provider(&provider_id);
                            ...
                            .on_click(cx.listener(move |this, _, window, cx| {
                                ...
                                workspace.update(cx, |workspace, cx| {
                                    workspace.toggle_modal(window, cx, |window, cx| {
                                        AgentApiKeyModal::new(title, configuration_view, window, cx)
                                    });
                                });
                            }))
                    }))
        ...
        let content = match &self.server_state {
            ServerState::Loading { _loading, .. } => {
                ...
                let label_text = self
                    .loading_status
                    .clone()
                    .or_else(|| android_agent_loading_label(&loading.agent_id))
...
pub(crate) mod tests {
    ...
    async fn test_restored_threads_keep_available_commands(cx: &mut TestAppContext) {
        ...
        assert_eq!(
            placeholder,
            Some("Message Test â€” @ to include context, / for commands".to_string())
        );
    ...
    async fn test_thread_search_shift_enter_navigates_with_jetbrains_keymap(
        ...
        assert!(
            initial_count >= 2,
            "test precondition: need â‰¥2 matches across the thread, got {}",
            initial_count,
        );
        assert_eq!(
            bar.read_with(cx, |bar, _| bar.active_match_index()),
    ...
    async fn test_thread_search_navigates_from_outside_search_bar(cx: &mut TestAppContext) {
        ...
        assert!(
            initial_count >= 2,
            "test precondition: need â‰¥ 2 matches, got {}",
            initial_count,
        );
        assert_eq!(
            bar.read_with(cx, |bar, _| bar.active_match_index()),
    ...
    fn resolve_outcome_from_selection_dropdown_defaults_to_last_choice_when_no_selection() {
        ...
        // Last choice is "Only this time" â†’ option_id "allow".
        assert_eq!(outcome.option_id.0.as_ref(), "allow");
        assert_eq!(outcome.option_kind, acp::PermissionOptionKind::AllowOnce);
    }
    ...
    fn resolve_outcome_from_selection_dropdown_out_of_range_falls_back_to_last() {
        ...
        // choices.get(999) is None, falls back to choices.last() â†’ "Only this time".
        assert_eq!(outcome.option_id.0.as_ref(), "allow");
    }
    ...
    async fn test_move_queued_message_to_empty_main_editor(cx: &mut TestAppContext) {
        ...
        active_thread(&conversation_view, cx).update_in(cx, |thread, window, cx| {
            ...
            // Main editor must be empty for this path â€” it is by default, but
