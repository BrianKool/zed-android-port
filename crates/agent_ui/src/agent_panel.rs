...
impl AgentPanel {
    ...
    pub fn new_external_agent_thread(
        ...
        // The `+` menu always starts a blank conversation. History is the
        // only path that should restore an existing Codex or Claude session.
        self.selected_agent = Agent::from(action.agent.clone());
        self.activate_new_thread(true, AgentThreadSource::AgentPanel, window, cx);
    }
