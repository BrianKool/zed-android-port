...
use gpui::{
    ...
    ReadGlobal as _, Role, ScrollHandle, Stateful, Subscription, Task, TitlebarOptions,
...
fn init_renderers(cx: &mut App) {
    cx.default_global::<SettingFieldRenderer>()
        ...
        .add_basic_renderer::<settings::AgentOutputLanguage>(render_dropdown)
    ...
    if let Some(workspace_handle) = workspace_handle {
        ...
        cx.defer(move |cx| {
            if let Err(err) = workspace_handle.update(cx, move |multi_workspace, window, cx| {
                let callback = callback.take().expect("settings callback must run once");
                ...
                multi_workspace.toggle_modal(window, cx, |window, cx| {
                    ...
                    callback(&mut settings_window, window, cx);
                    settings_window
                });
            }) {
...
impl SettingsWindow {
    ...
    fn render_nav(
        ...
        v_flex()
            ...
            .when(!compact, |this| {
                this.w(SIDEBAR_WIDTH).h_full().border_r_1()
            })
            .p_2p5()
