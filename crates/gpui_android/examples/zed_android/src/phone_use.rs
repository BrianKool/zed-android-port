...
use gpui::{
    ...
    ScrollHandle, Task, WeakEntity, Window, prelude::*,
};
use theme::ActiveTheme;
use ui::{
    Button, Clickable, Color, Disableable, Headline, HeadlineSize, Icon, IconName, IconSize, Label,
    LabelCommon, LabelSize, ParentElement, ScrollAxes, Scrollbars, Styled, WithScrollbar, h_flex,
    v_flex,
};
        ...
        v_flex()
            ...
            .child(
                v_flex()
                    ...
                    .p_4()
                    .gap_4()
                    .child(Headline::new("Browser & Phone Use").size(HeadlineSize::Medium))
                    .child(Label::new("Browser Use coordinates both layers automatically: Crawl4AI reads public pages efficiently, while Droid-MCP controls and verifies the real Android Chrome session. Phone Use always uses Droid-MCP.").color(Color::Muted))
                    .child(
                        h_flex()
                            .gap_2()
                            .child(Icon::new(if ready { IconName::Check } else { IconName::Warning }).color(if ready { Color::Success } else { Color::Warning }))
                            .child(Label::new(self.status.clone()).color(if ready { Color::Success } else { Color::Warning })),
                    )
