...
fn ensure_android_browser_bridge(home: &std::path::Path) -> Result<PathBuf> {
    let browser_launcher = home.join(".local/bin/zdroid-open-url");
    let parent = browser_launcher
        .parent()
        .context("Android browser bridge has no parent")?;
    std::fs::create_dir_all(parent).context("create Android browser bridge directory")?;
    std::fs::write(
        &browser_launcher,
        r#"#!/system/bin/sh
set -eu
...
if [ -z "$url" ]; then
    /system/bin/log -t zed_android_browser "Ignored empty OAuth URL request" || true
    exit 2
fi
/system/bin/log -t zed_android_browser "Opening OAuth URL through Zdroid bridge: ${url%%\?*}" || true
...
"#,
    ...
    for helper in ["termux-open-url", "xdg-open"] {
        let helper_launcher = home.join(format!(".local/bin/{helper}"));
        std::fs::write(
            &helper_launcher,
            format!(
                "#!/system/bin/sh\n/system/bin/log -t zed_android_browser 'Redirecting {helper} through Zdroid' || true\nexec {} \"$@\"\n",
                browser_launcher.to_string_lossy()
            ),
        )
        .with_context(|| format!("write {helper} compatibility bridge"))?;
        let mut permissions = std::fs::metadata(&helper_launcher)?.permissions();
        permissions.set_mode(0o700);
        std::fs::set_permissions(&helper_launcher, permissions)
            .with_context(|| format!("chmod {helper} compatibility bridge"))?;
    }

    Ok(browser_launcher)
}

fn manual_auth_environment() -> &'static str {
    r#"export NO_BROWSER=true
unset BROWSER
unset DISPLAY
"#
}
...
fn ensure_codex_acp_launcher() -> Result<PathBuf> {
    ...
    ensure_android_browser_bridge(&home)?;
    let manual_auth_environment = manual_auth_environment();
...
setup_status="$setup_dir/codex-acp.status"
...
if [ ! -x "$codex_bin" ]; then
    set_setup_status 'Downloading Codex CLI'
    ...
    if mkdir "$lock" 2>/dev/null; then
        ...
        set_setup_status 'Installing Codex CLI'
        rm -rf "$root"
...
if [ ! -x "$codex_bin" ]; then
    set_setup_status 'Codex CLI installation failed'
    echo "Zdroid-B: Android Codex installation did not complete." >&2
    exit 127
fi

set_setup_status 'Starting Codex agent'
...
exec "$codex_bin" "$@"
"#,
        manual_auth_environment = manual_auth_environment,
...
managed_cli="$root/node_modules/@anthropic-ai/claude-code/cli.js"
global_acp="$PREFIX/lib/node_modules/@agentclientprotocol/claude-agent-acp/dist/index.js"
...
if [ -f "$managed_acp" ]; then
    resolved_acp="$managed_acp"
elif [ -f "$global_acp" ]; then
    resolved_acp="$global_acp"
else
    resolved_acp=""
fi
...
if [ -z "$resolved_cli" ] || [ -z "$resolved_acp" ]; then
    ...
    if [ -d "$lock" ]; then
        owner="$(cat "$lock/pid" 2>/dev/null || true)"
        if [ -z "$owner" ] || [ ! -r "/proc/$owner/stat" ]; then
            rm -rf "$lock"
        fi
    fi
    ...
    if mkdir "$lock" 2>/dev/null; then
        ...
        set_setup_status 'Waiting for Claude CLI installation'
        waited=0
        while [ ! -f "$managed_cli" ] && [ "$waited" -lt 900 ]; do
            sleep 1
            waited=$((waited + 1))
        done
    fi
...
if [ ! -f "$resolved_cli" ] || [ ! -f "$resolved_acp" ]; then
    set_setup_status 'Claude CLI installation failed'
    echo 'Zdroid-B: Claude CLI or Claude ACP installation did not complete.' >&2
    exit 127
fi
...
set_setup_status 'Starting Claude agent'
...
const RETIRED_ANDROID_ACP_AGENTS: &[&str] =
    &["gemini", "github-copilot-cli", "grok-build", "opencode"];
...
fn boot(cx: &mut App, data_path: &std::path::Path, dns_resolver: AndroidDnsResolver) -> Result<()> {
    ...
    let bootstrap_node = data_path.join("usr/bin/node");
    let bootstrap_npm = data_path.join("usr/bin/npm");
    cx.observe_global::<SettingsStore>(move |cx| {
        let settings = &project::project_settings::ProjectSettings::get_global(cx).node;
        let configured_paths = settings.path.as_ref().map(|node_path| {
        ...
        let options = node_runtime::NodeBinaryOptions {
            ...
            use_paths: configured_paths
                .or_else(|| Some((bootstrap_node.clone(), bootstrap_npm.clone()))),
        };
        node_options_tx.send(Some(options)).log_err();
    })
    .detach();
    let node_runtime = NodeRuntime::new(client.http_client(), None, node_options_rx);
    info!("zed_android: RealFs + NodeRuntime constructed (Bootstrap Node + PATH lookup)");
