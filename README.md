...
2. **Runtime setup.** Choose **Zdroid Standard** for the lightweight Android-native environment or **Zdroid Full Linux** to add an Ubuntu glibc userland for compatible ARM64 Linux software and local servers.
3. **Bootstrap setup.** The app downloads, verifies, extracts, and repairs its Termux-derived userland with visible progress. The foreground session notification and optional WakeLock allow setup to continue in the background; later launches reuse the installed runtime.
